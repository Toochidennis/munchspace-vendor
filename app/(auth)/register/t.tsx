...
const setupSchema = z.object({
  ...
  city: z.string().min(1, "City is required."), // ← changed from fullAddress
  ...
  const handleNext = async () => {
    ...
    if (step === 4) {
      ...
      try {
        ...
        // Transform working hours to array format that API expects
        const transformedWorkingHours = daysOfWeek
          .filter((day) => values.workingHours[day]?.enabled)
          .map((day) => ({
            day: day.toUpperCase(),
        ...
        // Now sending as array — this should fix both errors
        formData.append(
          "workingHours",
            ...
            >
              ...
              {step === 4 && (
                <div className="space-y-6">
                  ...
                  <FormField
                    control={form.control}
                    name="city" // ← changed from fullAddress
                    render={({ field }) => (
                      <FormItem>
                        <FormLabel className="font-normal text-slate-500">
